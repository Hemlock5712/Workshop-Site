...
test("stripping an AI utm_source parameter → no error", () => {
  const before =
    "Background: https://example.com/post?utm_source=chatgpt.com and more.";
...
test("ordinary prose rewrite → no error", () => {
  const before =
    "We leverage a robust and comprehensive framework to delve into the landscape of developer tooling.";
...
test("rewrite that adds AI patterns → residual error", () => {
  const before =
    "The team shipped the parser on Tuesday. It reads 40MB files without the old memory spike.";
  const after =
...
test("rewrite that removes AI patterns → clean", () => {
  ...
  const after =
    "We use a reliable framework to study developer tooling. The team built it in six weeks.";
  const r = validate(before, after);
  assert.equal(r.ok, true, formatResult(r));
