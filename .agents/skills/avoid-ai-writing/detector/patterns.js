...
const AIDetector = (() => {
  ...
  const TIER1_PHRASES = [
    ...
    {
      pattern: /\bmarking\s+a\s+pivotal\s+moment\b/gi,
      replace: "state what happened",
    },
    {
      pattern: /\bthe\s+future\s+looks\s+bright\b/gi,
      replace: "cut or say something specific",
    },
    {
      pattern: /\bonly\s+time\s+will\s+tell\b/gi,
      replace: "cut or say something specific",
    },
    {
    ...
    {
      pattern: /\bthought\s+leader(?:ship)?\b/gi,
      replace: "expert, authority",
    },
    {
      pattern: /\bbest\s+practices\b/gi,
      replace: "what works, proven methods",
    },
    ...
    {
      pattern: /\bfeatures\b/gi,
      replace: "has, includes",
      filter: true,
      clarity: true,
    },
    ...
    {
      pattern: /\bcommenc(?:e|es|ing|ed)\b/gi,
      replace: "start, begin",
      clarity: true,
    },
    {
      pattern: /\bascertain(?:s|ing|ed)?\b/gi,
      replace: "find out, determine",
      clarity: true,
    },
    {
      pattern: /\bendeavou?r(?:s|ing|ed)?\b/gi,
      replace: "effort, attempt, try",
      clarity: true,
    },
    ...
    {
      pattern:
        /\bload-bearing\b(?!\s+(?:(?:structural|exterior|interior|internal|external|concrete|steel|timber|wooden|brick|masonry|perimeter|basement|main|primary|existing|original)\s+)?(?:walls?|beams?|columns?|joists?|truss(?:es)?|members?|footings?|slabs?|studs?|partitions?|masonry|lintels?|piers?|rafters?|girders?|capacity|capacities)\b)/gi,
  ...
  function maskYamlFrontmatter(chars) {
    ...
    if (first !== "---" || lines.length < 2 || /^\s*$/.test(bare(lines[1])))
      return;
    ...
    for (let i = 0; i < chars.length; i += 1) {
      if (start === -1) {
        if (
          chars[i] === open &&
          !isEscaped(i) &&
          (!apostropheAware || !isWord(chars[i - 1]))
        )
          start = i;
      } else if (
        chars[i] === close &&
  ...
  function maskHyphenationProtected(text) {
    ...
    maskMatches(
      /(?:[a-z]:[\\/]|\.{1,2}[\\/]|~[\\/]|[\\/])[a-z0-9_.-]{1,255}/gi
    );
    maskMatches(
      /(?:[a-z]:[\\/]|(?:\.\.?[\\/])?)(?:[a-z0-9_.-]{1,64}[\\/]){1,128}[a-z0-9_.-]{1,64}/gi
    );
    ...
    maskMatches(
      /@[a-z0-9_.-]{1,64}\/[a-z0-9_.-]{1,64}-[a-z0-9_.-]{1,64}(?:@[^\s,;)\]}]{1,32})?/gi
    );
    maskMatches(
      /\b[a-z0-9_.-]{1,64}-[a-z0-9_.-]{1,64}@[~^]?v?\d[a-z0-9*_.+-]{0,31}\b/gi
    );
    maskMatches(
      /\b(?:[a-z0-9_.-]{0,64}\d[a-z0-9_.-]{0,64}-[a-z0-9_.-]{1,64}|[a-z0-9_.-]{1,64}-[a-z0-9_.-]{0,64}\d[a-z0-9_.-]{0,64})\b/gi
    );
    maskMatches(
      /\b[a-z_][a-z0-9_.]{0,63}(?:-[a-z0-9_.]{1,64}){1,8}(?=[ \t]*[=:])/gi
    );
    maskMatches(
  ...
  function isSocialTag(tag) {
    return (
      !/^\d+$/.test(tag) && !HEX_COLOUR.test(tag) && !CPP_DIRECTIVE.test(tag)
    );
  }
  ...
  const UNNECESSARY_HYPHENATION = [
    ...
    // a project-specific spelling of the pair in another role is not swept in.
    {
      ...
      suggestion: (match) =>
        match.replace("research-impact", "research impact"),
    },
    {
    ...
    {
      pattern: /\bRust-crate\s+usage\b/g,
      suggestion: (match) => match.replace("Rust-crate", "Rust crate"),
    },
    {
    ...
    },
    {
      ...
      suggestion: (match) =>
        match.replace("total-downloads", "total downloads"),
    },
    {
    ...
    {
      pattern: /\bcode-base\b/g,
      suggestion: (match) => match.replace("-", ""),
    },
    { pattern: /\bdata-set\b/g, suggestion: (match) => match.replace("-", "") },
    {
      pattern: /\btime-frame\b/g,
      suggestion: (match) => match.replace("-", ""),
    },
  ...
  function analyzeText(text, options = {}) {
    ...
    const contextMode = VALID_CONTEXT_MODES.has(requestedMode)
      ? requestedMode
      : "general";
    const contextModeFallback =
      requestedMode !== contextMode ? requestedMode : null;
    ...
    issues.push(
      ...matchPatterns(text, CHATBOT_ARTIFACTS, "chatbot", "critical")
    );
    ...
    issues.push(
      ...matchPatterns(text, LETS_PATTERNS, "lets-construction", "medium")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    issues.push(
      ...matchPatterns(text, EMOTIONAL_FLATLINE, "emotional-flatline", "low")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    issues.push(
      ...matchPatterns(text, NOVELTY_INFLATION, "novelty-inflation", "medium")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    issues.push(
      ...matchPatterns(text, AI_PLACEHOLDERS, "ai-placeholder", "critical")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    issues.push(
      ...matchPatterns(text, AI_UTM_SOURCE, "ai-utm-source", "critical")
    );
    issues.push(
      ...matchPatterns(text, TEMPLATE_PHRASES, "template-phrase", "high")
    );
    issues.push(
      ...matchPatterns(text, FALSE_CONCESSION, "false-concession", "medium")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    issues.push(
      ...matchPatterns(text, FUTURE_NARRATIVE, "future-narrative", "high")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    issues.push(
      ...matchPatterns(text, SOCIAL_CTA_CLOSER, "social-cta-closer", "high")
    );
    issues.push(...findUnnecessaryHyphenation(text));

    // ── Tier 1 v2: formulaic openers + parenthetical hedges ──────────
    issues.push(
      ...matchPatterns(text, FORMULAIC_OPENERS, "formulaic-opener", "high")
    );
    issues.push(
      ...matchPatterns(text, SPECULATIVE_OPENERS, "speculative-opener", "high")
    );
    issues.push(
      ...matchPatterns(
        text,
    ...
    // dashes are typography and don't corroborate it.
    {
      ...
      const separatorEmDashes =
        (text.match(SEPARATOR_DASH_RE) || []).length +
      ...
      const signals = [hasCurly, hasEmDash, hasOxford, clean].filter(
        Boolean
      ).length;
    ...
    if (wordCount >= 150) {
      ...
      if (seq.length >= 50) {
        ...
        const normalized =
          distinctCount > 1 ? entropy / Math.log2(distinctCount) : 1;
    ...
    if (paragraphs.length >= 4) {
      ...
      if (cvs.length >= 4) {
        ...
        const cvVar =
          cvs.reduce((s, c) => s + (c - cvMean) ** 2, 0) / cvs.length;
    ...
    const hashtagMatches = [
      ...maskCode(text).matchAll(/(?:^|\W)#(\w[\w-]*)/g),
    ...
    if (hashtagMatches.length >= 6) {
      issues.push({
        ...
        severity: "medium",
        suggestion:
          "Cut to 2-3 specific tags or none. Long hashtag blocks read as bot output.",
      });
    }
    ...
    const confIssues = matchPatterns(
      text,
  ...
  function buildSentenceRegions(text, issues) {
    ...
    Object.defineProperty(regions, "_unmapped", {
      value: unmappedHighlights,
      enumerable: false,
    });
    return regions;
  }
  ...
  function classifyTrinary({
    ...
    const hasSmartPunct = issues.some(
      (i) => i.type === "smart-punct-signature"
    );
    ...
    const aiSoft = Math.min(
      0.97,
      score / 100 + totalCorrob * 0.06 + strongCorrob * 0.08
    );
    let p;
    if (classification === "HUMAN_ONLY")
      ...
      p = {
        human: Math.max(0.15, 0.6 - aiSoft * 0.5),
        mixed: 0.5,
        ai: aiSoft * 0.7,
      };
