...
test("AI-heavy paragraph scores in Strong/Heavy range", () => {
  ...
  assert.ok(
    ["Strong AI signals", "Heavy AI patterns"].includes(r.label),
    `got label: ${r.label}`
  );
});
...
test("stats fields sum to issues length", () => {
  ...
  const sum =
    ...
    r.stats.tier2Count +
    r.stats.tier3Count +
    r.stats.patternCount;
  assert.equal(
    sum,
    r.issues.length,
    `stats sum (${sum}) != issues (${r.issues.length})`
  );
});

test("repeated Tier 1 phrase does not inflate score linearly", () => {
  const single = AIDetector.analyzeText(
    "We delve into the landscape of many things today."
  );
...
test("em-dash detector skips definition-list separators (bold term / link + dash)", () => {
  ...
  assert.equal(
    emDashIssues.length,
    0,
    "separator-position dashes should not count toward the rate"
  );
});
...
test("em-dash carve-out covers numbered-list separators too", () => {
  ...
  assert.equal(
    emDashIssues.length,
    0,
    "numbered-list separators should not count toward the rate"
  );
});
...
test("hedge-stack does not fire on ordinary negation or inverted questions (#69)", () => {
  ...
  const frame = (s) =>
    `The committee reviewed the proposal at length and concluded that it ${s} work as designed, given every constraint documented during the previous quarter.`;
  for (const phrase of [
    "could not possibly",
    "could never possibly",
    "could a savage possibly",
    "might a person conceivably",
  ]) {
    const r = AIDetector.analyzeText(frame(phrase));
    assert.equal(
      r.issues.filter((i) => i.type === "hedge-stack").length,
      0,
      `"${phrase}" should not fire`
    );
  }
  for (const phrase of [
    "could potentially",
    "may eventually unlock",
    "might ultimately transform",
  ]) {
    const r = AIDetector.analyzeText(frame(phrase));
    assert.ok(
      r.issues.filter((i) => i.type === "hedge-stack").length >= 1,
      `"${phrase}" should still fire`
    );
  }
});
...
test("em-dash carve-out requires a list marker — line-initial bold splices still fire", () => {
  ...
  const emDashIssues = r.issues.filter((i) => i.type === "em-dash");
  assert.ok(
    emDashIssues.length >= 1,
    "bold-lead splices outside a list should still count"
  );
});
...
test("smart-punct signature is not corroborated by separator-only em dashes", () => {
  ...
  assert.equal(
    hits.length,
    0,
    "separator-only dashes should not complete the co-occurrence signature"
  );
});
...
test("em-dash detector ignores CLI flags like --save-dev", () => {
  ...
  assert.equal(
    emDashIssues.length,
    0,
    "CLI flags should not count as em dashes"
  );
});
...
test("chatbot artifacts score as P0 critical", () => {
  ...
  assert.ok(
    chatbotIssues.length >= 2,
    `expected chatbot detections, got ${chatbotIssues.length}`
  );
...
#AI #Crypto #Blockchain #DePIN #Mining #Web3 #Solana #RandomX #DecentralizedAI #PassiveIncome #Infrastructure #Innovation #Technology #FutureTech #Tokenomics`;
  ...
  assert.ok(
    ...
    "expected tier3-phrase-cluster flag"
  );
  assert.ok(
    r.score >= 25,
    `expected score ≥25 (Some/Moderate), got ${r.score}`
  );
});
...
test('"Interesting part of the project:" header opener flags emotional-flatline', () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("emotional-flatline"),
    "expected emotional-flatline flag"
  );
});

test('"the line I keep coming back to" flags lingering-attention', () => {
  const text =
    "Recorded with a guest yesterday. The line I keep coming back to is that agents behave like teenagers on an unbounded goal.";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("lingering-attention"),
    "expected lingering-attention flag"
  );
});

test('"I cannot stop thinking about" flags lingering-attention', () => {
  const text =
    "I can't stop thinking about the runtime guardrail argument he made near the end of our conversation about agent drift.";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("lingering-attention"),
...
test('bare "I keep coming back to X because ..." does NOT flag lingering-attention', () => {
  ...
  const text =
    "I keep coming back to the exit-voice framing because it predicts which engineers quit and which ones file the RFC instead.";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    !types.has("lingering-attention"),
    "bare reasoned form must not flag"
  );
});
...
test('"real on-chain tokenomics" flags real-actual-inflation', () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("real-actual-inflation"),
    "expected real-actual-inflation flag"
  );
});
...
test("hashtag-stuff does not fire on prose with 2-3 hashtags", () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    !types.has("hashtag-stuff"),
    "should not flag 2 hashtags as hashtag-stuff"
  );
});
...
test('"load-bearing" (metaphor) flags tier1; construction nouns exempt', () => {
  ...
  const lbHits = (text) => {
    ...
    assert.ok(
      !r.tooShort,
      `fixture must clear the length gate (wordCount >= 10): ${text}`
    );
    return {
      hits: r.issues.filter((i) => /load[- ]bearing/i.test(i.text)),
      types: new Set(r.issues.map((i) => i.type)),
    };
  };
  ...
  for (const text of literals) {
    ...
    assert.equal(
      hits.length,
      0,
      `literal construction use should not fire tier1: ${text}`
    );
  }
});
...
test("#107: deterministic unnecessary hyphenation subclasses fire with fixes", () => {
  const cases = [
    [
      "The team built a research-impact aggregator for the annual reporting workflow.",
      "research-impact aggregator",
      "research impact aggregator",
    ],
    [
      "The report summarizes two research-impact aggregations from the external evaluation teams.",
      "research-impact aggregations",
      "research impact aggregations",
    ],
    [
    ...
    [
      "The guide compares Python-package usage across the supported deployment environments.",
      "Python-package usage",
      "Python package usage",
    ],
    [
      "The guide also compares Rust-crate usage across the supported deployment environments.",
      "Rust-crate usage",
      "Rust crate usage",
    ],
    [
      "The repository keeps a single-Project Manifest for every supported deployment environment.",
      "single-Project Manifest",
      "single Project Manifest",
    ],
    [
      "The audit records a total-downloads figure for every published package each month.",
      "total-downloads figure",
      "total downloads figure",
    ],
    [
      "The dashboard reports a life-sciences-native citation count beside each indexed article.",
      "life-sciences-native citation count",
      "citation count from a life sciences source",
    ],
    [
    ...
    [
      "- Migration note\n    The old code-base remains available while customers finish moving their applications.",
      "code-base",
      "codebase",
    ],
    [
      "The old data-set still feeds the internal dashboard used by the support team.",
      "data-set",
      "dataset",
    ],
    [
      "The published time-frame leaves enough room for another review before launch.",
      "time-frame",
      "timeframe",
    ],
    [
      "The product road-map lists every migration milestone planned for the next quarter.",
      "road-map",
      "roadmap",
    ],
    [
      "The service refreshes the dashboard in real-time, even during the nightly import.",
      "in real-time",
      "in real time",
    ],
    [
      "The service refreshes the dashboard in real-time every day during the nightly import.",
      "in real-time",
      "in real time",
    ],
    [
      "The service refreshes the dashboard in real-time continuously during the nightly import.",
      "in real-time",
      "in real time",
    ],
    [
      "The service refreshes the dashboard in real-time as new records arrive for processing.",
      "in real-time",
      "in real time",
    ],
    [
      "The service refreshes the dashboard in real-time via the existing event stream.",
      "in real-time",
      "in real time",
    ],
    [
      "The current release works out-of-the-box on every operating system we support.",
      "works out-of-the-box",
      "works out of the box",
    ],
    [
      "That shortcut creates maintenance problems over the long-term, despite its early convenience.",
      "over the long-term",
      "over the long term",
    ],
    [
      "That shortcut creates maintenance problems over the long-term across every department.",
      "over the long-term",
      "over the long term",
    ],
    [
      "That shortcut creates maintenance problems over the long-term through deferred upgrades.",
      "over the long-term",
      "over the long term",
    ],
    [
      "The team plans to keep this compatibility layer for the long-term by design.",
      "for the long-term",
      "for the long term",
    ],
  ];
  ...
  for (const [text, matched, suggestion] of cases) {
    ...
    const hits = result.issues.filter(
      (issue) => issue.type === "unnecessary-hyphenation"
    );
    assert.equal(
      hits.length,
      1,
      `expected one unnecessary-hyphenation hit: ${text}`
    );
    assert.equal(hits[0].text, matched, `unexpected matched text: ${text}`);
    assert.equal(
      hits[0].suggestion,
      suggestion,
      `unexpected suggestion: ${text}`
    );
    assert.equal(
      hits[0].severity,
      "medium",
      `hyphenation cleanup should be P2: ${text}`
    );
  }
});
...
test("#107: standard compound modifiers and open forms do not fire", () => {
  ...
  for (const text of clean) {
    ...
    assert.equal(
      hits.length,
      0,
      `legitimate compound should stay clean: ${text}`
    );
  }
});
...
test("#107: protected spans are excluded from unnecessary-hyphenation detection", () => {
  ...
  assert.equal(
    hits.length,
    0,
    `protected text should not fire: ${JSON.stringify(hits)}`
  );
});
...
test("#107: proper nouns, identifiers, and version strings stay protected", () => {
  ...
  for (const text of protectedForms) {
    const hits = AIDetector.analyzeText(text).issues.filter(
      (issue) => issue.type === "unnecessary-hyphenation"
    );
    assert.equal(
      hits.length,
      0,
      `proper noun or identifier should stay protected: ${text}`
    );
  }
  ...
  assert.equal(
    prose.issues.filter((issue) => issue.type === "unnecessary-hyphenation")
      .length,
    1,
...
test("#107: frontmatter, YAML, Markdown tables, and HTML attributes stay protected", () => {
  ...
  assert.equal(
    hits.length,
    0,
    `metadata should stay protected: ${JSON.stringify(hits)}`
  );
...
test("#107: adversarial filename masking remains within a linear-time budget", () => {
  const attacks = [`${"a-".repeat(3000)}a`, `${"segment/".repeat(1000)}`];
  for (const attack of attacks) {
    ...
    assert.ok(
      elapsedMs < 1000,
      `adversarial mask scan took ${elapsedMs.toFixed(1)}ms`
    );
  }
});
...
test("#107: long closed quotations are masked without a length cutoff", () => {
  ...
  assert.equal(
    hits.length,
    0,
    "quoted material must stay protected below the analyzer word limit"
  );
});

test("#107: copyedit-only findings do not affect score or trinary classification", () => {
  const clean =
    "The team reviewed the release notes before publishing them to customers. Everyone checked the examples, links, headings, and migration steps before the final approval meeting.";
  const copyedits =
    "The code-base and data-set updates follow the time-frame in the road-map. The service runs in real-time, works out-of-the-box, and remains supported over the long-term for every customer.";
  const baseline = AIDetector.analyzeText(clean);
  const result = AIDetector.analyzeText(copyedits);

  assert.ok(
    result.issues.filter((issue) => issue.type === "unnecessary-hyphenation")
      .length >= 7,
  ...
  assert.equal(
    result.score,
    baseline.score,
    "copyedit-only issues must not change the AI score"
  );
  assert.equal(
    result.label,
    baseline.label,
    "copyedit-only issues must not change the AI label"
  );
  assert.equal(
    result.document_classification,
    baseline.document_classification
  );
...
test('"verbatim" is Tier 3: single use stays clean, overuse flags by density', () => {
  ...
  assert.equal(
    single.issues.filter((i) => i.type === "tier3" && /verbatim/i.test(i.text))
      .length,
    0,
  ...
  assert.equal(
    termOfArt.issues.filter(
      (i) => i.type === "tier3" && /verbatim/i.test(i.text)
    ).length,
    0,
  ...
  const hits = overused.issues.filter(
    (i) => i.type === "tier3" && /verbatim/i.test(i.text)
  );
  assert.ok(
    hits.length > 0,
    'repeated "verbatim" uses in a short piece should flag tier3 density'
  );
});
...
test('"quietly" clusters with another Tier 2 word flags tier2', () => {
  ...
  const single = AIDetector.analyzeText(
    "The team quietly shipped the update last week without any announcement."
  );
  const singleTypes = new Set(single.issues.map((i) => i.type));
  assert.ok(
    !singleTypes.has("tier2"),
    'single "quietly" should not fire tier2 on its own'
  );

  const clustered = AIDetector.analyzeText(
    "The team quietly worked to harness new opportunities, building the platform without any announcement."
  );
  const clusteredTypes = new Set(clustered.issues.map((i) => i.type));
  assert.ok(
    clusteredTypes.has("tier2"),
    'expected tier2 flag when "quietly" clusters with another Tier 2 word'
  );
});
...
test('"deeply" joins a tier2 cluster only in significance collocations', () => {
  ...
  for (const text of literal) {
    ...
    assert.ok(
      !types.has("tier2"),
      `literal "deeply" must not cluster: ${text}`
    );
  }
  ...
  const clustered = AIDetector.analyzeText(
    "The team is deeply committed to helping partners harness the new platform across every region."
  );
  const clusteredTypes = new Set(clustered.issues.map((i) => i.type));
  assert.ok(
    clusteredTypes.has("tier2"),
    'expected tier2 when "deeply committed" clusters with another Tier 2 word'
  );

  // ...but the collocation alone, with no second Tier 2 word, stays clean.
  const lone = AIDetector.analyzeText(
    "The billing module is deeply integrated with the ledger, so invoice edits show up in both places."
  );
  const loneTypes = new Set(lone.issues.map((i) => i.type));
  assert.ok(
    !loneTypes.has("tier2"),
    'lone "deeply integrated" must not fire tier2 by itself'
  );
});
...
That's the full list for this push.`;
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    !types.has("bullet-np-list"),
    "verb-form bullets should not trip bare-NP detector"
  );
});
...
test("tier3-phrase fires on per-phrase repetition (>=2 hits)", () => {
  ...
  assert.ok(
    ...
    "expected tier3-phrase flag for 2x repetition"
  );
});
...
test("tier3-phrase span dedup: overlapping regex matches count as one phrase", () => {
  ...
  const text =
    "This protocol is designed for long-term sustainability and nothing else.";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    !types.has("tier3-phrase-cluster"),
...
test("emotional-flatline opener fires at position 0 (no leading newline)", () => {
  ...
  assert.ok(
    types.has("emotional-flatline"),
    "expected emotional-flatline at position 0"
  );
});
...
test("bullet-np-list ignores bullets inside fenced code blocks", () => {
  ...
  assert.ok(
    !types.has("bullet-np-list"),
    "bullets inside code fences should not flag"
  );
});
...
test("bullet-np-list flushes on 2+ blank lines between bullet sections", () => {
  ...
  assert.ok(
    !types.has("bullet-np-list"),
    "sections separated by 2+ blank lines should not merge"
  );
});
...
test("hashtag-stuff matches tags after sentence punctuation", () => {
  ...
  assert.ok(
    types.has("hashtag-stuff"),
    "expected hashtag-stuff on 7-tag trailing block"
  );
});
...
test("hashtag-stuff excludes URL fragments from the count", () => {
  ...
  assert.ok(
    !types.has("hashtag-stuff"),
    "URL fragments should not count as hashtags"
  );
});
...
test("hashtag-stuff excludes issue and PR references from the count", () => {
  ...
  const text =
    "The regression came in through #88 and stayed hidden until #91 landed. I reverted #92, reopened #93, and then #94 turned out to be the same bug in a different file. #95 is the follow up that fixes it, and #96 tracks the test we still owe.";
  const types = new Set(AIDetector.analyzeText(text).issues.map((i) => i.type));
  assert.ok(
    !types.has("hashtag-stuff"),
    "issue references should not count as hashtags"
  );
});
...
test("hashtag-stuff excludes hex colours and preprocessor directives", () => {
  ...
  const css =
    "Background is #fff in light mode and #eee in dark. Body text sits at #1a2b3c, muted text at #6b7280, the link colour is #2563eb, hover is #1d4ed8, and the one accent is #f59e0b on the button.";
  // 8-character RGBA values, so dropping the {8} alternative is caught.
  const rgba =
    "Palette is #1a2b3cff for body and #6b7280ee muted and #2563ebdd links and #1d4ed8cc hover and #f59e0bbb accent and #0a1b2cdd border today.";
  const c =
    "Put #include <stdio.h> first, then #include <stdlib.h>, then #include <string.h>. Add #include <unistd.h> and #include <fcntl.h> after those, and guard the block with #ifndef and #endif so it stays idempotent.";
  for (const [label, text] of [
    ...
    const types = new Set(
      AIDetector.analyzeText(text).issues.map((i) => i.type)
    );
    assert.ok(
      !types.has("hashtag-stuff"),
      `${label} should not count as hashtags`
    );
  }
});
...
test("hashtag-stuff ignores hashes inside code spans and fences", () => {
  ...
  const inline =
    "The escape rules trip people up. Write `#AI` for the tag, `#Innovation` for the category, `#Startups` for the vertical, `#Leadership` for the theme, `#Growth` for the metric, and `#FutureOfWork` when you mean the movement.";
  const fenced =
    "Here is the config we ship by default, and it has not changed in a year:\n\n```\n#alpha\n#beta\n#gamma\n#delta\n#epsilon\n#zeta\n```\n\nEverything below that line is user overridable and nothing above it is.";
  for (const [label, text] of [
    ["inline code", inline],
    ["fenced code", fenced],
  ]) {
...
test("hashtag-stuff still counts short hex-shaped words, which are real tags", () => {
  ...
  const gtm =
    "Great conversation on the go-to-market motion this week with the whole revenue team here.\n#b2b #e2e #saas #growth #ace #fade";
  const family =
    "Weekend was good and the whole family got outside for once this month together.\n#dad #cafe #beef #face #travel #weekend";
  const decade =
    "Reflecting on the last ten years of shipping developer tools to teams everywhere here.\n#decade #facade #deface #beaded #effaced #growth";
  // Four spaces under a list marker is a paragraph continuation, not a code
  // block, which is why indented runs are not masked.
  const listed =
    "- We shipped the detector and the whole team is happy with how it landed today.\n\n    #AI #Innovation #FutureOfWork #MachineLearning #Leadership #Growth";
  for (const [label, text] of [
    ["b2b/e2e block", gtm],
    ["dad/cafe block", family],
    ["a-f word tags", decade],
    ["tags under a list item", listed],
  ]) {
    const types = new Set(
      AIDetector.analyzeText(text).issues.map((i) => i.type)
    );
    assert.ok(
      types.has("hashtag-stuff"),
      `${label} should still flag as hashtag stuffing`
    );
  }
});
...
test("hashtag-stuff still fires on a tag block that also cites an issue", () => {
  ...
  const text =
    "Closed out #88 today and the release is live for everyone.\n#AI #Innovation #FutureOfWork #MachineLearning #Leadership #Growth #Startups";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("hashtag-stuff"),
    "expected hashtag-stuff on a 7-tag block"
  );
  assert.equal(
...
test("hashtag-stuff still fires on tags spread inline through a post", () => {
  ...
  const text =
    "Loving the #AI space right now, especially #MachineLearning and #DeepLearning, plus #Startups and #Innovation and #FutureOfWork keep me busy every single day of the week.";
  const types = new Set(AIDetector.analyzeText(text).issues.map((i) => i.type));
  assert.ok(
    types.has("hashtag-stuff"),
    "expected hashtag-stuff on 6 inline tags"
  );
});
...
test("low-ttr fires on a 200+ token text with narrow vocabulary", () => {
  ...
  const sentence =
    "The system shows the system improves the system every iteration. ";
  const text = sentence.repeat(20);
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("low-ttr"),
    `expected low-ttr flag, got types: ${[...types].join(", ")}`
  );
});
...
test("ai-placeholder fires on common slot-fill bracket patterns", () => {
  ...
  for (const text of [
    ...
    assert.ok(
      ...
      `expected ai-placeholder for: ${text}`
    );
  }
});
...
test("ai-citation-markup fires on chatbot-internal tokens", () => {
  ...
  for (const text of [
    ...
    const r = AIDetector.analyzeText(
      text +
        " ".repeat(0) +
        "Padding text to clear the word-count gate. ".repeat(2)
    );
    const types = new Set(r.issues.map((i) => i.type));
    assert.ok(
...
test("ai-utm-source fires on AI-tool tracking parameters", () => {
  ...
  for (const text of [
    ...
    const types = new Set(r.issues.map((i) => i.type));
    assert.ok(
      types.has("ai-utm-source"),
      `expected ai-utm-source for: ${text.slice(0, 60)}...`
    );
  }
});
...
test("ai-utm-source does not fire on benign utm_source values", () => {
  ...
  assert.ok(
    !types.has("ai-utm-source"),
    "newsletter UTM should not flag as AI source"
  );
});
...
test("v2: zero-width chars trigger normalization-flag", () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("normalization-flag"),
    "expected normalization-flag on ZWSP injection"
  );
  assert.ok(
    r.stats.normalization.zeroWidth > 0,
    "norm.zeroWidth should count strip"
  );
});
...
test("v2: Cyrillic homoglyph swap restores Tier 1 hit", () => {
  ...
  assert.ok(
    types.has("normalization-flag"),
    "expected normalization-flag on homoglyph cluster"
  );
  assert.ok(
...
test("v2: speculative scenario opener fires", () => {
  const text =
    "Imagine a world where every developer ships bug-free code on the first try. That is the promise this framework keeps making in its docs, and it deserves a much harder look before anyone commits a roadmap to it.";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("speculative-opener"),
    "expected speculative-opener flag"
  );

  // The comma-interrupted cadence is the same tell and must also fire.
  const interrupted = AIDetector.analyzeText(
    "Imagine, for a moment, a world where every deploy is instant. That is the pitch, and the pricing page leans on it hard enough that the claim deserves scrutiny before anyone signs."
  );
  const interruptedTypes = new Set(interrupted.issues.map((i) => i.type));
  assert.ok(
    interruptedTypes.has("speculative-opener"),
    'expected speculative-opener on "Imagine, for a moment, a world where"'
  );
});
...
test('v2: speculative opener leaves instructional "imagine you have" alone', () => {
  ...
  for (const text of clean) {
    ...
    assert.ok(
      !types.has("speculative-opener"),
      `false positive on instructional prose: ${text}`
    );
  }
});
...
test("v2: parenthetical hedge fires", () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("parenthetical-hedge"),
    "expected parenthetical-hedge flag"
  );
});

test("v2: social endorsement closer fires on LinkedIn-style share post", () => {
  const text =
    "Just finished Sarah's deep dive on why context windows leak in long agent runs. She walks through the eviction policy line by line and shows where the tokens actually go. This one is worth your time:";
  const r = AIDetector.analyzeText(text);
  const types = new Set(r.issues.map((i) => i.type));
...
test("v2: social endorsement closer covers every regex branch", () => {
  ...
  for (const text of variants) {
    ...
    const types = new Set(r.issues.map((i) => i.type));
    assert.ok(
      types.has("social-cta-closer"),
      `expected social-cta-closer on: ${text}`
    );
  }
});
...
test("v2: social endorsement closer matches curly-apostrophe forms", () => {
  ...
  for (const text of curly) {
    ...
    assert.ok(
      types.has("social-cta-closer"),
      `expected social-cta-closer on curly form: ${text}`
    );
  }
});
...
test("v2: social endorsement closer leaves literal-verb human prose alone", () => {
  ...
  for (const text of clean) {
    ...
    assert.ok(
      !types.has("social-cta-closer"),
      `false positive on literal prose: ${text}`
    );
  }
});
...
test("v2: trinary output present + FN-biased for ambiguous text", () => {
  ...
  assert.ok(
    ...
    "expected document_classification field"
  );
  assert.ok(
  ...
  const sum =
    r.class_probabilities.human +
    r.class_probabilities.mixed +
    r.class_probabilities.ai;
  assert.ok(
    Math.abs(sum - 1) < 0.02,
...
test("v2: highlight_sentence_for_ai returns regions with start/end offsets", () => {
  ...
  const r = AIDetector.analyzeText(text);
  assert.ok(
    Array.isArray(r.highlight_sentence_for_ai),
    "expected highlight array"
  );
...
test('v2: context mode "technical" suppresses Title Case header flag', () => {
  ...
  const technicalHas = technical.issues.some(
...
test("#62: Title Case flagged on a Markdown heading, not just a bare line", () => {
  ...
  for (const prefix of ["#", "##", "######"]) {
    const r = AIDetector.analyzeText(
      `${prefix} Strategic Negotiations And Key Partnerships${body}`,
      {
        contextMode: "general",
      }
    );
...
test("#62: the heading fix does not flag sentence-case or non-headings", () => {
  ...
  const cases = [
    [
      "## Strategic negotiations and key partnerships",
      "sentence-case heading is correct, not a tell",
    ],
    [
      "##Strategic Negotiations And Key Partnerships",
      "no space after # is not a Markdown heading",
    ],
    [
      "####### Strategic Negotiations And Key Partnerships",
...
test("#62: legitimate three-word Title Case headings are not flagged", () => {
  ...
  for (const heading of [
    ...
    assert.equal(
      titleCaseHits(heading + HEADING_BODY).length,
      0,
      `must not flag: ${heading}`
    );
  }
});
...
test("#62: fences that a parity count gets wrong", () => {
  ...
  const f4 = "````";
  const intro =
    "Documentation about writing Markdown, long enough to clear the word gate.";
  ...
  assert.equal(
    titleCaseHits(
      [intro, f4, f3 + "markdown", title, f3, f4].join("\n") + HEADING_BODY
    ).length,
    0,
...
test("#77: only spaces and tabs may follow a closing fence", () => {
  ...
  for (const fence of ["```", "~~~"]) {
    assert.equal(
      titleCaseHits(
        [intro, fence, fence + "js", title, fence].join("\n") + HEADING_BODY
      ).length,
      0,
    ...
    assert.equal(
      titleCaseHits(
        [intro, fence, "code", fence + " \t", title].join("\n") + HEADING_BODY
      ).length,
      1,
...
test("#62: MD_HEADING_PREFIX accepts what the pattern accepts", () => {
  ...
  assert.equal(
    titleCaseHits("##\tBenefits And Strategic Considerations" + HEADING_BODY)
      .length,
    1,
...
test("#62: the reported text is value-asserted, not merely present", () => {
  ...
  const hits = titleCaseHits(
    "## Benefits And Strategic Considerations" + HEADING_BODY
  );
...
test("v2: punct-distribution fires on uniform per-paragraph density", () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("punct-distribution"),
    "expected punct-distribution flag on uniform density"
  );
});
...
test("v2: cross-para-burstiness fires on uniform sentence rhythm", () => {
  ...
  const types = new Set(r.issues.map((i) => i.type));
  assert.ok(
    types.has("cross-para-burstiness"),
    "expected cross-para-burstiness on uniform rhythm"
  );
});
...
test("v2: invalid contextMode falls back to general with stats.contextModeFallback set", () => {
  ...
  assert.equal(
    r.stats.contextMode,
    "general",
    "invalid mode coerced to general"
  );
  assert.equal(
    r.stats.contextModeFallback,
    "tecnical",
    "fallback echoes original"
  );
});
...
test("v2: trinary fields present on tooShort / tooLong / empty as UNSCORED", () => {
  ...
  assert.equal(
    tooShort.stats.contextMode,
    "general",
    "tooShort stats includes contextMode"
  );
  assert.equal(
    tooLong.stats.contextMode,
    "general",
    "tooLong stats includes contextMode"
  );
});
...
test("v2: mid-score isolated stylometric hits do not reach AI_ONLY", () => {
  ...
  assert.ok(
    !hasCutoff,
    "precondition: corpus should not trigger cutoff-disclaimer"
  );
  assert.ok(
    !hasReasonChat,
    "precondition: corpus should not trigger reasoning+chatbot"
  );
  assert.ok(
    !hasNorm,
    "precondition: corpus should not trigger normalization-flag"
  );
  assert.ok(
    r.score < 70,
    `precondition: corpus score should be < 70, got ${r.score}`
  );
...
test("v2: legitimate *italic phrase* is NOT stripped by roleplay rule", () => {
  ...
  const text =
    "We use *italic phrase here* for emphasis and *another phrase too* in some places.";
  const norm = AIDetector.normalizeText(text);
  assert.equal(
    norm.flags.roleplay,
    0,
...
test("v2: *roleplay action verb* IS stripped", () => {
  ...
  const norm = AIDetector.normalizeText(text);
  assert.ok(
    norm.flags.roleplay >= 2,
    `expected ≥2 roleplay strips, got ${norm.flags.roleplay}`
  );
});
...
test("v2: blockquoted AI text does not penalize the human wrapper", () => {
  ...
  const r = AIDetector.analyzeText(text);
  assert.ok(
    r.stats.quotedLines >= 3,
    `expected quotedLines >= 3, got ${r.stats.quotedLines}`
  );
  assert.notEqual(
    r.document_classification,
    "AI_ONLY",
...
test("v2: unmappedHighlights counter surfaced in stats", () => {
  ...
  assert.equal(
    typeof r.stats.unmappedHighlights,
    "number",
    "unmappedHighlights should be numeric"
  );
});
...
test("v2: stats.denseAIVocab and stats.tier1Distinct surface for observability", () => {
  ...
  assert.equal(
    typeof r.stats.denseAIVocab,
    "boolean",
    "denseAIVocab should be boolean"
  );
  assert.equal(
    typeof r.stats.tier1Distinct,
    "number",
    "tier1Distinct should be number"
  );
});
...
test("#109: Object.prototype names in prose do not fire tier lookups", () => {
  ...
  const r = AIDetector.analyzeText(text);
  const protoHits = r.issues.filter((i) =>
    [
      "constructor",
      "prototype",
      "tostring",
      "valueof",
      "hasownproperty",
    ].includes(String(i.text).toLowerCase())
  );
  assert.equal(
    protoHits.length,
    0,
    `prototype-name tokens flagged: ${JSON.stringify(protoHits)}`
  );
});
...
test("#109 complement: real tier1 vocabulary still fires after the hasOwn guard", () => {
  ...
  const tier1Texts = r.issues
    .filter((i) => i.type === "tier1")
    .map((i) => String(i.text).toLowerCase());
  assert.ok(
    tier1Texts.includes("delve"),
    `expected 'delve' to still fire, got tier1=${JSON.stringify(tier1Texts)}`
  );
  assert.ok(
    !tier1Texts.includes("constructor"),
    "constructor must not ride along in tier1"
  );
});
