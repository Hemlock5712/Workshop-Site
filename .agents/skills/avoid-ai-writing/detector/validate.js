...
const AIDetectorValidate = (() => {
  ...
  const PATH =
    /(?:^|[\s(])((?:\.{0,2}\/)[A-Za-z0-9._~\-]+(?:\/[A-Za-z0-9._~\-]+)*|[A-Za-z]:\\[A-Za-z0-9._\\~\-]+)/g;
  ...
  function sample(list, n = 5) {
    ...
    return (
      shown.join(", ") + (list.length > n ? ` (+${list.length - n} more)` : "")
    );
  }
  ...
  function validate(original, rewritten, options = {}) {
    ...
    if (typeof original !== "string" || typeof rewritten !== "string") {
      throw new TypeError(
        "validate(original, rewritten): both arguments must be strings"
      );
    }
    ...
    if (origFenced.length !== newFenced.length) {
      err(
        "code-block-count",
        `Fenced code blocks changed in number: ${origFenced.length} → ${newFenced.length}.`
      );
    } else if (origFenced.some((block, i) => block !== newFenced[i])) {
      const changed = origFenced.findIndex(
        (block, i) => block !== newFenced[i]
      );
      err(
        "code-block-modified",
        `Fenced code block #${changed + 1} was modified.`
      );
    }
    ...
    if ((origYaml ? origYaml[0] : null) !== (newYaml ? newYaml[0] : null)) {
      err(
        ...
        "YAML frontmatter was modified, added, or removed."
      );
    }
    ...
    const origQuotes = extractAll(BLOCKQUOTE_BLOCK, origProse).map(
      normalizeQuote
    );
    const newQuotes = extractAll(BLOCKQUOTE_BLOCK, newProse).map(
      normalizeQuote
    );
    ...
    if (lostTables.length) {
      err(
        "table-modified",
        `Markdown table content was modified or removed (${lostTables.length} table(s)).`
      );
    }

    // ── Inline code: identifiers, flags, filenames. ──
    const lostInline = missingFrom(
      extractAll(INLINE_CODE, original),
      extractAll(INLINE_CODE, rewritten)
    );
    ...
    // ── Filesystem paths. ──
    const lostPaths = missingFrom(
      extractAll(PATH, origProse),
      extractAll(PATH, newProse)
    );
    if (lostPaths.length) {
    ...
    const origHeadings = [];
    const newHeadings = [];
    for (const [, hashes, text] of original.matchAll(MD_HEADING))
      origHeadings.push({ level: hashes.length, text });
    for (const [, hashes, text] of rewritten.matchAll(MD_HEADING))
      newHeadings.push({ level: hashes.length, text });
    if (origHeadings.length !== newHeadings.length) {
      ...
      const levelDrift = origHeadings.findIndex(
        (h, i) => h.level !== newHeadings[i].level
      );
      if (levelDrift !== -1) {
        err(
          "heading-level",
      ...
      const reworded = origHeadings.filter(
        (h, i) => h.text !== newHeadings[i].text
      );
      if (reworded.length) {
        warn(
          "heading-text",
    ...
    const lostNumbers = missingFrom(
      extractAll(NUMBER, origProse),
      extractAll(NUMBER, newProse)
    );
    ...
    const maxShrink =
      options.maxShrinkRatio == null ? 0.4 : options.maxShrinkRatio;
  ...
  function formatResult(result) {
    ...
    lines.push(
      result.ok
        ? "PASS — preservation checks clear"
        : `FAIL — ${result.errors.length} preservation error(s)`
    );
...
if (
  ...
  if (!origPath || !newPath) {
    console.error(
      "usage: node detector/validate.js <original-file> <rewritten-file>"
    );
    process.exit(2);
  }
