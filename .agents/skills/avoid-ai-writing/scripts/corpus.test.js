...
test("extraction throws rather than returning empty text", () => {
  assert.throws(
    () => htmlToText("<div>no article</div>", { selector: "article" }),
    /no <article>/
  );
  assert.throws(
    () => htmlToText("<div>no paras</div>", { mode: "paragraphs" }),
    /no <p> elements/
  );
});
...
test("entities are decoded and tags removed", () => {
  const html =
    "<article><p>Ben &amp; Jerry&#8217;s &#8212; a test &lt;here&gt;</p></article>";
  const out = htmlToText(html, { selector: "article" });
...
test("em dashes survive extraction", () => {
  ...
  const html =
    "<article><p>One thing &#8212; and another — plus a third.</p></article>";
  const out = htmlToText(html, { selector: "article" });
...
test("script and style contents never reach the text", () => {
  const html =
    "<article><script>var delve = 1;</script><style>.a{color:red}</style><p>Body.</p></article>";
  const out = htmlToText(html, { selector: "article" });
...
test("slice.after starts at the marker", () => {
  const out = applySlice("Front matter here. MARKER the real text begins.", {
    after: "MARKER",
  });
...
test("slice.after throws on a missing marker rather than silently taking everything", () => {
  assert.throws(
    () => applySlice("some text", { after: "NOPE" }),
    /marker not found/
  );
});
...
test("raid: only defensible registers are mapped", () => {
  ...
  assert.equal(
    DOMAIN_REGISTER.german,
    undefined,
    "non-English domains are out of scope"
  );
});
