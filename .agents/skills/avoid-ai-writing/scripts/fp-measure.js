  ...
  for (const [g, list] of [...groups.entries()].sort()) {
    ...
    for (const t of THRESHOLDS) {
      ...
      out[g][t] = {
        n: list.length,
        flagged,
        rate: list.length ? flagged / list.length : 0,
        ci: [lo, hi],
      };
    }
  }
...
function summarize({ units, skipped, unit }) {
  ...
  for (const t of THRESHOLDS) {
    ...
    overall[t] = {
      fpr: {
        n: human.length,
        flagged: fp,
        rate: human.length ? fp / human.length : 0,
        ci: wilson(fp, human.length),
      },
      tpr: {
        n: machine.length,
        flagged: tp,
  ...
  return {
    ...
    bySource,
    byRegister: {
      human: rateTable(human, "register"),
      machine: rateTable(machine, "register"),
    },
...
function report(s, units) {
  ...
  console.log(
    "Human units are human by provenance; machine units are labelled by RAID."
  );
  console.log(
    "A flag on a human unit is a false positive. A flag on a machine unit is a true positive.\n"
  );
  ...
  for (const t of THRESHOLDS) {
    ...
    console.log(
      `  score >= ${String(t).padEnd(3)}` +
        `${pct(o.fpr.rate).padStart(7)} (${pct(o.fpr.ci[0])}–${pct(o.fpr.ci[1])})`.padEnd(
          26
        ) +
  ...
  for (const [model, rows] of Object.entries(s.byModel)) {
    const r = rows[25];
    console.log(
      `    ${model.padEnd(16)} n=${String(r.n).padStart(4)}   ${pct(r.rate).padStart(6)} at >=25`
    );
  }

  console.log("\n  By register (>=25)");
  const regs = new Set([
    ...Object.keys(s.byRegister.human),
    ...Object.keys(s.byRegister.machine),
  ]);
...
function main() {
  ...
  if (args.includes("--json")) {
    console.log(
      JSON.stringify(
        { generated_by: "scripts/fp-measure.js", thresholds: THRESHOLDS, ...s },
        null,
        2
      )
    );
