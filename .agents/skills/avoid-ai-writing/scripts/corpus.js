...
function htmlToText(html, opts = {}) {
  ...
  if (config.mode === "paragraphs") {
    ...
    const paras = [...html.matchAll(/<p\b([^>]*)>([\s\S]*?)<\/p>/gi)]
      .filter(([, attrs]) =>
        config.match ? attrs.includes(config.match) : true
      )
      .map(([, , body]) => body);
    if (!paras.length)
      throw new Error(
        `no <p> elements matched ${JSON.stringify(config.match || "any")}`
      );
    source = paras.join("\n\n");
  } else {
...
function applySlice(text, slice) {
  ...
  if (slice.after) {
    ...
    if (idx === -1)
      throw new Error(
        `slice.after marker not found: ${JSON.stringify(slice.after)}`
      );
...
async function fetchDoc(doc, force) {
  ...
  if (doc.source.type === "dataset") {
    ...
    if (!builders[doc.source.dataset])
      throw new Error(`unknown dataset: ${doc.source.dataset}`);
    const { build } = require(builders[doc.source.dataset]);
    const { jsonl, stats } = await build(doc.source.select, (m) =>
      console.error(m)
    );
...
async function cmdFetch(force) {
  ...
  const bad = results.filter(
    (r) => !["cached", "fetched", "local"].includes(r.status)
  );
...
function cmdVerify() {
  ...
  for (const doc of m.documents) {
    ...
    if (h !== doc.sha256) {
      console.log(
        `  DRIFT     ${doc.id} recorded ${doc.sha256.slice(0, 16)}, got ${h.slice(0, 16)}`
      );
      drift++;
    } else {
  ...
  if (drift) {
    ...
    console.error(
      "back is not reproducible until this is explained. Do not silently re-record."
    );
    return 1;
  }
  if (missing) {
...
function cmdList() {
  ...
  for (const reg of REGISTERS) {
    ...
    console.log(
      `  ${reg}  (${docs.length} docs, ${words.toLocaleString()} words)`
    );
    for (const d of docs) {
      const avail = loadText(d) === null ? " [unavailable]" : "";
      console.log(
        `    ${d.id.padEnd(34)} ${String(d.year || "").padEnd(6)} ${d.source.license}${avail}`
      );
    }
  }
...
function cmdAddLocal(args) {
  ...
  if (!id || !filePath) {
    console.error(
      "usage: node scripts/corpus.js add-local <id> <path> --register R --author A --year Y [--title T]"
    );
    return 2;
  }
...
async function main() {
  ...
  switch (cmd) {
    ...
    default:
      console.error(
        "usage: node scripts/corpus.js <list|fetch|verify|add-local>"
      );
      process.exit(2);
  }
}
