...
const isTitleCase = (h) =>
  majorWords(h).filter((b) => /^[A-Z]/.test(b)).length >= 2;
...
function check(text, mechanics) {
  ...
  for (const [k, v] of Object.entries(m)) {
    ...
    if (Array.isArray(spec)) {
      ...
      warnings.push({
        rule: "unknown-value",
        detail: `${k}: ${JSON.stringify(v)} (expected ${spec})`,
      });
  }
  ...
  const prose = lines.map((l, i) => {
    ...
    if (fm) {
      ...
      if (!inFence) {
        ...
        fenceChar = ch;
        fenceLen = len;
      } else if (ch === fenceChar && len >= fenceLen && /^\s*$/.test(fm[2]))
        inFence = false;
      paraBreak.push(false);
      inIndent = false;
      prevBlank = false;
  ...
  const advisory = [];
  const heads = prose
    .map((l, i) => [i + 1, l.match(/^#{1,6}\s+(.*)$/)])
    .filter(([, h]) => h);

  if (m.quotes === "straight") {
    prose.forEach((l, i) => {
      if (/[“”‘’]/.test(l))
        hard.push({ line: i + 1, rule: "quotes-should-be-straight" });
    });
  } else if (m.quotes === "curly") {
    prose.forEach((l, i) => {
      const c = l.replace(/(\d)['"]/g, "$1"); // carve out feet/inch primes (5'11")
      if (/"/.test(c))
        hard.push({ line: i + 1, rule: "double-quote-should-be-curly" });
  ...
  if (m.headings === "sentence") {
    heads.forEach(([ln, h]) => {
      if (isTitleCase(h[1]))
        advisory.push({ line: ln, rule: "heading-may-need-sentence-case" });
    });
  } else if (m.headings === "title") {
    heads.forEach(([ln, h]) => {
      if (looksSentenceCase(h[1]))
        advisory.push({ line: ln, rule: "heading-may-need-title-case" });
    });
  }
    ...
    prose.forEach((l, i) => {
      ...
      while ((mm = re.exec(l)) !== null) {
        ...
        const at =
          ...
          (before.match(/\)/g) || []).length;
        if (at <= 0)
          hard.push({ line: i + 1, rule: "latin-abbrev-outside-parens" });
      }
      depth = Math.max(
        0,
  ...
  if (m.emDash === "sparing") {
    ...
    if (em > Math.floor(words / 1000))
      advisory.push({
        rule: "em-dash-rate",
        detail: `${em} in ${words} words`,
      });
  }
  ...
  if (typeof m.spellNumbersUpTo === "number") {
    prose.forEach((l, i) => {
      ...
      const nums = (l.match(/(?<![\w.$:])\d{1,3}(?![\w.%:])/g) || []).filter(
        (n) => +n <= m.spellNumbersUpTo
      );
      if (nums.length)
        advisory.push({
          line: i + 1,
          rule: "number-may-need-spelling",
          detail: nums.join(", "),
        });
    });
  }
...
if (require.main === module) {
  ...
  if (!file || !cfgArg) {
    console.error(
      "usage: check-style.js <file> --config <config.json|name> [--json]"
    );
    process.exit(2);
  }
  ...
  if (json) {
    ...
    const w = r.warnings.length
      ? `, ${r.warnings.length} config warning(s)`
      : "";
    console.log(
      `${config.name || cfgPath}: ${r.hard.length} hard, ${r.advisory.length} advisory${w}`
    );
    r.warnings.forEach((x) => console.log(`  ! ${x.rule}: ${x.detail}`));
    r.hard.forEach((x) => console.log(`  L${x.line || "-"}  ${x.rule}`));
    r.advisory.forEach((x) =>
      console.log(
        `  L${x.line || "-"}  ${x.rule} (advisory)${x.detail ? `: ${x.detail}` : ""}`
      )
    );
  }
