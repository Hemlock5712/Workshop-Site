...
async function build(spec, log = () => {}) {
  ...
  for (const [file, cfg] of Object.entries(DOMAINS)) {
    ...
    for (const line of lines) {
      ...
      const human = (rec.human_answers || []).find(
        (a) => wordCount(a) >= minWords
      );
      const machine = (rec.chatgpt_answers || []).find(
        (a) => wordCount(a) >= minWords
      );
    ...
    log(
      `  ${file}: ${records.length} paired records available, ${take.length} taken`
    );
  }
  ...
  const warnings = [];
  if ((byClass.machine || 0) < 200)
    warnings.push(`${byClass.machine || 0} machine units (want >= 200)`);
  if (Object.keys(byDomain).length < 4)
    warnings.push(`only ${Object.keys(byDomain).length} domains`);
  warnings.push(
