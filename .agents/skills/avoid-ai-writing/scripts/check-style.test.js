...
t("quotes:straight flags curly marks; a clean doc passes", () => {
  assert.strictEqual(
    check('# ok\n\nplain "straight" text', { quotes: "straight" }).hard.length,
    0
  );
  assert.ok(
...
t("quotes:curly carves out feet/inch primes (5'11\")", () => {
  assert.strictEqual(
    check("The wall is 5'11\" tall.", { quotes: "curly" }).hard.length,
    0
  );
});
...
t("latinAbbrev:never flags any e.g./i.e.", () => {
  assert.ok(
    check("Retry, e.g. 3 times.", { latinAbbrev: "never" }).hard.some(
      (x) => x.rule === "latin-abbrev-not-allowed"
    )
  );
  assert.strictEqual(
    check("Retry, for example 3 times.", { latinAbbrev: "never" }).hard.length,
    0
  );
});
t(
  "latinAbbrev:parentheses flags outside parens, not inside (must-not-fire)",
  () => {
    assert.ok(
    ...
    assert.strictEqual(
      check("Retry a few times (e.g., 3).", { latinAbbrev: "parentheses" }).hard
        .length,
      0
    );
  }
...
t(
  "a title-case heading is advisory under headings:sentence, never hard",
  () => {
    ...
    assert.ok(
      r.advisory.some((x) => x.rule === "heading-may-need-sentence-case")
    );
  }
);
t("a proper-noun heading is NOT a hard violation", () => {
  ...
  assert.strictEqual(
    check("# Configure Docker and Kubernetes\n\ntext", { headings: "sentence" })
      .hard.length,
    0
  );
  assert.strictEqual(
    check("# Deploying to Google Cloud\n\ntext", { headings: "sentence" }).hard
      .length,
    0
  );
});
...
t("unknown key and unknown value are warned, not silently ignored", () => {
  ...
  assert.ok(
    r.warnings.some((x) => x.rule === "unknown-key" && x.detail === "quote")
  );
  assert.ok(
    r.warnings.some(
      (x) => x.rule === "unknown-value" && x.detail.startsWith("headings")
    )
  );
});
...
t(
  "frontmatter that closes is skipped; unterminated frontmatter does NOT swallow the doc",
  () => {
    assert.strictEqual(
      check('---\ntitle: "x"\n---\n# ok\n\nRun `git --force`.', {
        quotes: "straight",
    ...
    assert.ok(
      check("---\nx\n\nuse the “retry” option", {
        quotes: "straight",
      }).hard.some((x) => x.rule === "quotes-should-be-straight")
    );
  }
);
...
t(
  "markdown link titles and reference definitions are syntax, not curly-quote violations",
  () => {
    // Link titles MUST use straight quotes; flagging them hard-fails a correct document.
    assert.strictEqual(
      check('See the [docs](https://x.example "The Title") for more.', {
        quotes: "curly",
      }).hard.length,
      0
    );
    assert.strictEqual(
      check('[1]: https://x.example "Ref Title"', { quotes: "curly" }).hard
        .length,
      0
    );
    // A real straight quote in prose on the same line is still caught.
    assert.ok(
      check('He said "hi" in [docs](https://x.example "T").', {
        quotes: "curly",
      }).hard.length > 0
    );
  }
...
t(
  "a parenthetical that wraps across lines keeps its latinAbbrev carve-out",
  () => {
    const wrapped =
      "A long aside (this parenthetical wraps,\ne.g. across two lines) ends here.";
    assert.strictEqual(
      check(wrapped, { latinAbbrev: "parentheses" }).hard.length,
      0
    );
    ...
    assert.ok(
      ...
      }).hard.some((x) => x.rule === "latin-abbrev-outside-parens")
    );
  }
);
t(
  'a stray "(" does not suppress later latinAbbrev findings past the paragraph',
  () => {
    ...
    assert.ok(
      check(stray, { latinAbbrev: "parentheses" }).hard.some(
        (x) => x.rule === "latin-abbrev-outside-parens"
      )
    );
  }
...
t(
  "a link whose URL contains parentheses does not corrupt the paren balance",
  () => {
    // Regression: [^)]* mis-terminated on a nested paren and left a stray ")" behind.
    const wiki =
      "(We cite [Foo](https://en.wikipedia.org/wiki/Foo_(bar)), e.g. this one.)";
    assert.strictEqual(
      check(wiki, { latinAbbrev: "parentheses" }).hard.length,
      0
    );
    // ...and its straight-quoted title is still masked for quotes:curly.
    assert.strictEqual(
      check('[Foo](https://en.wikipedia.org/wiki/Foo_(bar) "The Title")', {
        quotes: "curly",
      }).hard.length,
      0
    );
...
t(
  ...
  () => {
    assert.ok(
      check('weird ]( text with "quotes" here.', { quotes: "curly" }).hard
        .length > 0
    );
  }
);
t(
  "prose that merely looks like a reference definition keeps its quotes checked",
  () => {
    assert.ok(
      check('[sic]: he said "hi" plainly.', { quotes: "curly" }).hard.length > 0
    );
  }
);
t(
  "a link-led definition-list line is not swallowed as a reference definition",
  () => {
    ...
    assert.ok(
      check('[Docs](https://d.example): "Note"', { quotes: "curly" }).hard
        .length > 0
    );
    assert.ok(
      check("[Docs](https://d.example): e.g. outside.", {
        latinAbbrev: "parentheses",
      }).hard.length > 0
    );
  }
...
t("a reference-definition title may contain escaped quotes", () => {
  assert.strictEqual(
    check('[1]: https://x.example "say \\"hi\\""', { quotes: "curly" }).hard
      .length,
    0
  );
});
t(
  "a parenthetical survives an intervening code block or code-only line",
  () => {
    // Regression: fenced/code-only lines mask to '' and were read as paragraph breaks,
    // resetting the paren depth mid-parenthetical.
    const fenced =
      "(See the config, for instance:\n```json\n{}\n```\nand, e.g., it closes here.)";
    assert.strictEqual(
      check(fenced, { latinAbbrev: "parentheses" }).hard.length,
      0
    );
    ...
    const gappy =
      "(See the config:\n```json\n{\n\n}\n```\nand, e.g., it closes here.)";
    assert.strictEqual(
      check(gappy, { latinAbbrev: "parentheses" }).hard.length,
      0
    );
    const inline =
      "(An aside about\n`some code`\ne.g. still inside the parens.)";
    assert.strictEqual(
      check(inline, { latinAbbrev: "parentheses" }).hard.length,
      0
    );
  }
);
t("HTML attribute values are syntax, not curly-quote violations", () => {
  assert.strictEqual(
    check('<div class="callout">\n\nText.', { quotes: "curly" }).hard.length,
    0
  );
  assert.strictEqual(
    check('<img alt="a chart" />', { quotes: "curly" }).hard.length,
    0
  );
});
t("prose containing a comparison is not mistaken for an HTML tag", () => {
  ...
  assert.ok(
    check('For n<N, the "tail" sum > epsilon.', { quotes: "curly" }).hard
      .length > 0
  );
  assert.ok(check("if a<b it isn't > c.", { quotes: "curly" }).hard.length > 0);
  assert.ok(
    check("When x<y (e.g. small) > z holds.", { latinAbbrev: "never" }).hard
      .length > 0
  );
});
t("a UTF-8 BOM does not hide the frontmatter", () => {
  assert.strictEqual(
    check('﻿---\ntitle: "x"\n---\nClean text.', { quotes: "curly" }).hard
      .length,
    0
  );
});
t(
  "nested and mismatched fences do not leak code as prose (no false hard violation)",
  () => {
...
t(
  "a bare name is not shadowed by a same-named file in the working directory",
  () => {
...
t("example configs are generic, parse with register + mechanics", () => {
  const tech = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "..", "examples", "technical.json"),
      "utf8"
    )
  );
  const prose = JSON.parse(
    fs.readFileSync(
      path.join(__dirname, "..", "examples", "prose.json"),
      "utf8"
    )
  );
  for (const cfg of [tech, prose]) {
    ...
    assert.ok(
      !/google|chicago|cmos|apa/i.test(cfg.name || ""),
      "name is guide-neutral"
    );
  }
  assert.strictEqual(tech.mechanics.latinAbbrev, "never");
  assert.strictEqual(prose.mechanics.latinAbbrev, "parentheses");
  assert.strictEqual(
    check("# Configure retries\n\nSet the value.", tech.mechanics).hard.length,
    0
  );
});
...
const cli = (mdText, cfgArg) => {
  const f = path.join(
    os.tmpdir(),
    `cs-${passed}-${Math.floor(process.hrtime()[1])}.md`
  );
  fs.writeFileSync(f, mdText);
  const r = spawnSync(
    "node",
    [path.join(__dirname, "check-style.js"), f, "--config", cfgArg],
    { encoding: "utf8" }
  );
  fs.unlinkSync(f);
  return r.status;
...
const cliRaw = (files, argv) => {
  ...
  const r = spawnSync(
    "node",
    [path.join(__dirname, "check-style.js"), ...argv],
...
t(
  "CLI: a second file, an unknown flag, and a missing --config value each exit 2",
  () => {
    assert.strictEqual(
      cliRaw({ "a.md": "# ok\n\ntext", "b.md": "# ok\n\ntext" }, [
        "a.md",
...
t(
  "CLI: a config whose mechanics is null or an array exits 2, not a false-green 0",
  () => {
    const withCfg = (cfgJson) => {
      ...
      const r = spawnSync(
        "node",
        [path.join(__dirname, "check-style.js"), "a.md", "--config", "c.json"],
        { cwd: dir, encoding: "utf8" }
      );
      fs.rmSync(dir, { recursive: true, force: true });
      return r.status;
...
t(
  "CLI: a file named like the config value is still reachable (not filtered by value)",
  () => {
...
t("4-space indented code after a blank line is masked (must-not-fire)", () => {
  ...
  assert.strictEqual(
    check("Para.\n\n\tx = f(a, e.g., b)\n\nMore.", { latinAbbrev: "never" })
      .hard.length,
    0
  );
});
t(
  "a 4-space line directly under a paragraph is lazy continuation and still checked",
  () => {
    assert.ok(
      check('Para starts\n    still "prose" here.', { quotes: "curly" }).hard
        .length > 0
    );
  }
);
t("4-space content inside a list item is prose and still checked", () => {
  assert.ok(
    check('- item\n\n    second "para" of the item', { quotes: "curly" }).hard
      .length > 0
  );
});
t(
  "a paragraph after a list resets list context, so indented code masks again",
  () => {
    assert.strictEqual(
      check('- item\n\nPlain paragraph.\n\n    code "x"\n\nAfter.', {
        quotes: "curly",
...
t(
  "a document opening with --- then a blank line is not frontmatter (must-fire)",
  () => {
    assert.ok(
      check('---\n\nHe said "hi".\n\n---\n\nmore', { quotes: "curly" }).hard
        .length > 0
    );
  }
);
