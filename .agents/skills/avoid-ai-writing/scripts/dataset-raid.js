...
const RAID_URL =
  "https://huggingface.co/datasets/liamdugan/raid/resolve/main/train.csv";
...
async function fetchWindow(offset) {
  ...
  if (!m) return [];
  return parseCsv(HEADER + raw.slice(m.index + 1), { dropLastPartial: true })
    .rows;
}
...
async function build(spec, log = () => {}) {
  ...
  for (const [i, off] of offsets(windows).entries()) {
    ...
    log(
      `  window ${i + 1}/${windows} at ${(off / 1e6).toFixed(0)}MB: ${rows.length} rows, ${kept} eligible`
    );
  }
  ...
  if (Object.keys(byDomain).length < 5) {
    warnings.push(
      `only ${Object.keys(byDomain).length} domains sampled (want >= 5)`
    );
  }
  if ((byClass.machine || 0) < 300) {
    warnings.push(
      `${byClass.machine || 0} machine units (want >= 300 for a +/-5pp interval)`
    );
  }
