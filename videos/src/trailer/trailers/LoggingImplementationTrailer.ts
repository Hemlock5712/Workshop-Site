...
export const LoggingImplementationTrailer: TrailerScript = {
  ...
  beats: [
    {
      id: "hook",
      text: "You turned logging on. Now make it worth opening. Three steps take a robot from silent to fully instrumented — and in Commands v3, telemetry has one new twist you need to know.",
      camera: TITLE,
      holdAfter: 0.5,
    },
    {
      id: "recipe",
      text: "The recipe: enable DataLogManager in the Robot constructor. Publish everything to NetworkTables — it's captured automatically. Then open the wpilog in AdvantageScope. That's the whole pipeline; the rest is doing it well.",
      camera: DIAGRAM,
      events: [
    ...
    {
      id: "publisher",
      text: "Here's a real v3 mechanism. The publisher is a field — a NetworkTables topic under Arm slash Position. Keep keys hierarchical like that, and AdvantageScope groups every mechanism into its own folder.",
      camera: CODE,
      events: [
        {
          type: "code-state",
    ...
    {
      id: "default-command",
      text: "But v3 mechanisms have no periodic method. So publish with a default command: runRepeatedly calls publishTelemetry whenever the arm is idle, and any real command simply pre-empts it. Telemetry for free, never in the way.",
      camera: CODE,
      events: [
        {
          type: "code-state",
    ...
    {
      id: "structs",
      text: "For structured data, don't scatter loose numbers. A StructPublisher sends a whole Pose2d — add velocities too, and one telemeterize call streams complete swerve state that drops straight onto AdvantageScope's field view.",
      camera: CODE2,
      events: [
        {
          type: "code-state",
    ...
    {
      id: "files",
      text: "Where do the files land? In simulation, a logs folder right in your project. On the SystemCore, a USB drive or the controller's logs directory. Either way, every run writes a wpilog you can replay.",
      camera: { x: 2600, y: 320, width: 2120, height: 1060 },
      holdAfter: 0.6,
    },
    {
      id: "cta",
      text: "Instrument once and every practice run becomes data you can replay. The full implementation — struct publishers, telemetry default commands, and reading your logs in AdvantageScope — is waiting at frc5712.com.",
      camera: END,
      holdAfter: 1.2,
    },
  ],
