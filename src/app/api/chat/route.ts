...
- Button bindings (\`onTrue\`/\`whileTrue\`) are unchanged from v2. Holds are bound with \`whileTrue\` (the team default): release the button, the hold is cancelled, and the mechanism's default command takes back over. NEVER bind a bare hold with \`onTrue\` — it runs forever and the default command never resumes; save \`onTrue\` for self-finishing commands
