...
Pose2d currentPose = drivetrain.getPose();`}
        ...
        >
          <p>
            ...
            <code>drivetrain.seedFieldCentric()</code>, on the left bumper: is a
            different operation: it re-zeroes the driver&apos;s idea of
      ...
      new SwerveRequest.ApplyFieldVelocity()
        ...
        >
          <li>
            ...
            <code>drivetrain::applyOperatorPerspective</code> running every
            loop. Its comment on the branch reads{" "}
            <em>
  ...
  protected boolean isFinished() {
    ...
    return false;
        ...
        <p className="prose-body-sm measure">
          ...
          <code>ClassicCommand</code> already returns by default: writing it out
          is a statement of intent for whoever reads the file next.
        </p>
    ...
    drivetrain.setControl(new SwerveRequest.Idle());
        ...
        <Box variant="concept" title="Why this line is not optional">
          ...
          <p className="mt-3">
            ...
            <code>TeleopOpMode</code> sets, and it takes the drivetrain back the
            instant <code>DriveToPoint</code> is canceled. With the sticks
          ...
          <p className="mt-3">
            ...
            <code>Mechanism.idle()</code> takes over at the lowest priority, and{" "}
            <code>idle()</code> sends no output at all and does not zero the
      ...
      >
        ...
        <CollapsibleSection title="Optional: a speed clamp you can add today">
          <p>
            ...
            replaces it properly. Add these two imports:{" "}
            <code>import static org.wpilib.units.Units.MetersPerSecond;</code>{" "}
            and <code>import frc.robot.generated.TunerConstants;</code>. Then
            add:
          </p>
          <div className="mt-3">
            <CodeBlock
...
double vy = clamp(yController.calculate(currentPose.getY(), targetPose.getY()), maxSpeed);`}
      ...
      <LessonSection id="two-problems-left-over" title="Two problems left over">
        ...
        <p>
          ...
          slow down, and then follows it. PID stops being the driver and becomes
          a small correction on top, which is why the gains drop from 10 / 10 /
          7 to 3.0 / 3.0 / 4.0 (also marked <code>TODO: tune</code> on that
          branch). And because the plan knows how long the trip takes, the
