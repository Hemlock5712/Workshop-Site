...
export const searchData: SearchItem[] = [
  ...
  },
  {
    ...
    "content": "(\"arm\"); return ( Mechanism Setup - Verifying Hardware Configuration Before implementing advanced control algorithms, we need to verify that motors and encoders are working correctly. This ensures proper direction, zeroing, and basic functionality. 🎯 Key Concept: Always verify hardware setup before adding control algorithms. Otherwise you will be debugging control issues when the problem is hardware configuration. Setup Steps by Mechanism 🦾 Arm Mechanism ⚡ Flywheel Mechanism 1️⃣ Encoder Direction You'll need to make sure your encoder is configured correctly. With the device facing you, as shown in the following picture, please make sure that the encoder position increases as you rotate the arm counterclockwise. Expected Behavior: • Counterclockwise rotation → Position increases • Clockwise rotation → Position decreases • Position values should be smooth and consistent 2️⃣ 🎯 Zero Encoder Position Set your mechanism to the zero position before running any control algorithms. For Arm mechanisms, zero should be straight parallel with the ground, facing the 3 o'clock position. Zero Position Setup: • Position Arm parallel to ground • Arm should point to 3 o'clock position • Reset encoder to 0.0 at this position • Verify position reading stays consistent Placeholder: Zero Position Image Placeholder: Zero Encoder Setup Video 3️⃣ Verifying Motor Setup You'll want to make sure your motor is spinning in the expected direction. If the motor is getting positive voltage, it should be spinning counterclockwise. You can check this through tuner, with the device facing you as in the following picture. 🔧 Motor Direction Verification Positive Voltage Test Apply +6V to your motor and observe movement direction. Expected: Positive voltage should move the mechanism in the \"positive\" direction (counter-clockwise for arms). Encoder Consistency Verify encoder readings match motor movement. Positive motor voltage → positive encoder change, negative motor voltage → negative encoder change. ) : ( 1️⃣ Verifying Motor Setup You'll want to make sure your flywheel motors are spinning in the expected direction. If the motor is getting positive voltage, both motors should spin in the same direction (leader-follower setup). You can check this through tuner. • Leader and follower motors spin together • Positive voltage → consistent rotation direction • No unusual noise or vibration • Smooth acceleration and deceleration Apply +6V to your flywheel and observe rotation. Both motors should rotate smoothly in the same direction with consistent speed. Leader-Follower Check Verify follower motor tracks leader motor exactly. Follower motor should mirror leader motor movements with minimal lag. ✅ Ready for Control Once your mechanism moves in the correct direction and provides accurate feedback (encoder for arms, motor consistency for flywheels), you're ready to implement advanced control algorithms. 💡 Next Step: With verified hardware setup, we can now implement PID control for precise mechanism control. The control algorithm will use sensor feedback to automatically reach target positions or velocities. use client arm flywheel Mechanism Setup /running-program Running Program /pid-control PID Control bg-focus-50 dark:bg-focus-900/20 text-[var(--foreground)] rounded-lg p-8 border border-focus-200 dark:border-focus-800 text-3xl font-bold text-slate-900 dark:text-slate-100 mb-4 text-[var(--muted-foreground)] mb-4 bg-learn-100 dark:bg-learn-900/30 p-4 rounded-lg text-learn-800 dark:text-learn-300 font-medium flex flex-col gap-8 text-3xl font-bold text-slate-900 dark:text-slate-100 card border-b border-[var(--border)] flex border-primary-600 text-primary-600 border-transparent text-[var(--muted-foreground)] hover:text-[var(--foreground)] p-6 card p-8 text-2xl font-bold text-[var(--foreground)] mb-6 grid grid-cols-3 gap-6 mb-8 col-span-2 bg-primary-50 dark:bg-primary-950/30 p-4 rounded-lg font-semibold text-primary-900 dark:text-primary-300 mb-2 text-primary-800 dark:text-primary-300 space-y-1 text-sm flex w-full justify-center /images/mechanisms/arm.jpg Arm mechanism for encoder verification rounded-lg https://www.youtube.com/embed/zJgSQKrz8yE Encoder Setup and Verification accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture w-full aspect-video rounded-lg bg-[var(--card)] text-[var(--foreground)] border border-[var(--border)] rounded-lg p-6 text-xl font-bold mb-4 grid md:grid-cols-2 gap-6 mb-6 mb-4 bg-concept-100 dark:bg-concept-900/30 p-4 rounded-lg font-semibold text-concept-900 dark:text-concept-300 mb-2 text-concept-800 dark:text-concept-300 space-y-1 text-sm w-full h-48 bg-concept-100 dark:bg-concept-900/50 rounded-lg flex items-center justify-center border border-concept-200 dark:border-concept-800 text-concept-600 dark:text-concept-400 text-sm w-full h-64 bg-concept-100 dark:bg-concept-900/50 rounded-lg flex items-center justify-center border border-concept-200 dark:border-concept-800 col-span-2 text-[var(--muted-foreground)] Arm https://www.youtube.com/embed/iQqR1Wxptzg Motor Testing mt-8 text-xl font-bold text-learn-600 mb-4 grid md:grid-cols-2 gap-6 bg-[var(--card)] text-[var(--foreground)] p-6 rounded-lg font-semibold mb-3 text-sm mb-3 bg-[var(--muted)] text-[var(--muted-foreground)] p-3 rounded border border-[var(--border)] text-xs text-[var(--muted-foreground)] /images/mechanisms/flywheel.png Flywheel mechanism for motor verification bg-[var(--muted)] border border-[var(--border)] rounded-lg p-6 text-xl font-bold text-[var(--foreground)] mb-4 bg-indigo-50 dark:bg-indigo-950/30 p-4 rounded mt-4 text-indigo-800 dark:text-indigo-300 text-sm",
    "url": "/mechanism-setup",
    "category": "Workshop 1",
    "tags": [
      "mechanism-setup",
  ...
  },
  {
    ...
    "content": "Running Program - Hardware Simulation Testing WPILib provides a powerful tool called Hardware Simulation. This allows you to run your code in the simulator, while also running motors that are connected to the CANivore. 🎯 Key Concept: Hardware simulation eliminates the need to use a roboRIO for testing, while still allowing you to test your code on hardware. Hardware Simulation Setup Running Program /adding-commands Commands /mechanism-setup Mechanism Setup bg-focus-50 dark:bg-focus-900/20 text-[var(--foreground)] rounded-lg p-8 border border-focus-200 dark:border-focus-800 text-3xl font-bold text-slate-900 dark:text-slate-100 mb-4 text-[var(--muted-foreground)] mb-4 bg-learn-100 dark:bg-learn-900/30 p-4 rounded-lg text-learn-800 dark:text-learn-300 font-medium flex flex-col gap-8 text-3xl font-bold text-slate-900 dark:text-slate-100 https://www.youtube.com/embed/xsR7m6ToUFE Hardware Simulation accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture w-full h-full aspect-video rounded-lg",
    "url": "/running-program",
    "category": "Workshop 1",
    "tags": [
      "running-program",
