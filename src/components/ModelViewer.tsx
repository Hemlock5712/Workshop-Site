...
function CameraTracker({ onPositionChange }: { onPositionChange: (position: [number, number, number]) => void }) {
  useFrame(({ camera }) => {
...
export default function ModelViewer({ url, width = 800, height = 600, className = "" }: ModelViewerProps) {
  const [error, setError] = useState<string | null>(null);
  const [loading, setLoading] = useState(true);
  const [cameraPosition, setCameraPosition] = useState<[number, number, number]>([100, 489, 530]);
