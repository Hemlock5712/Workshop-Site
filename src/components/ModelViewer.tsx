...
const ModelViewer = forwardRef<ModelViewerRef, ModelViewerPropsWithReset>(
  function ModelViewer({ url, className = "", showResetButton = false }, ref) {
    ...
    useImperativeHandle(ref, () => ({
      resetCamera: () => {
        if (orbitControlsRef.current) {
          orbitControlsRef.current.object.position.set(...initialPosition);
          orbitControlsRef.current.target.set(0, -100, 0);
